...
public class SphereTest {
    ...
    public void testFindIntersections() {
        ...
        assertEquals("Ray crosses sphere wrong", List.of(new Intersectable.GeoPoint(sphere, p1), new Intersectable.GeoPoint(sphere, p2)), result);

        // TC03: Ray starts inside the sphere (1 point)
        assertEquals("Not good when ray starts inside the sphere", List.of(new Intersectable.GeoPoint(sphere, new Point3D(1, 0.5, Math.sqrt(0.75)))),
        ...
        assertEquals("Not good when ray starts at the sphere", List.of(new Intersectable.GeoPoint(sphere, new Point3D(1, 0,1))),
        ...
        if (result.get(0).point.get_y().get() > result.get(1).point.get_y().get())
            result = List.of(result.get(1), result.get(0));
        assertEquals("Ray crosses sphere wrong when through the center", List.of(new Intersectable.GeoPoint(sphere, p1), new Intersectable.GeoPoint(sphere, p2)), result);
        // TC14: Ray starts at sphere and goes inside (1 points)
        assertEquals("Not good when ray starts at the sphere throw center", List.of(new Intersectable.GeoPoint(sphere, new Point3D(1, 1,0))),
                sphere.findIntersections(new Ray(new Point3D(1, -1, 0), new Vector(0, 1, 0))));
        // TC15: Ray starts inside (1 points)
        assertEquals("Not good when ray starts inside the sphere throw center", List.of(new Intersectable.GeoPoint(sphere, new Point3D(1, 1, 0))),
                sphere.findIntersections(new Ray(new Point3D(1, -0.5, 0), new Vector(0, 1, 0))));
        // TC16: Ray starts at the center (1 points)
        assertEquals("Not good when ray starts on the center", List.of(new Intersectable.GeoPoint(sphere, new Point3D(1, 1, 0))),
